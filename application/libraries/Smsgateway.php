...
class Smsgateway
{
    ...
    {
        //API URL
        $old_url    = "http://nimbusit.info/api/pushsms.php?user=106257&key=0107o40W30DG6WY1f5wF&sender=LEVASL&mobile=$to&text=Leva Shubhmangal Business Directory One Time Password $otp &entityid=1701162796587643226&templateid=1707165216349825729";
        $url        = str_replace(' ', '%20', $old_url);
        $curl       = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => $url,
            CURLOPT_RETURNTRANSFER => true,
        ...
        if ($err) {
            ...
            if (strpos($response, 'Message Sent')) {
                return TRUE;
            }
            return FALSE;
        }
